        ...
        public MainWindow() {
            ...
            timers = new DispatcherTimer(
               TimeSpan.FromMilliseconds(2000),
                DispatcherPriority.Normal,
                (sender, args) => GetUpThree());
            timers.Start();
        }
//------------------------------------------------------------------------------
        public async void GetUpThree() {
            timesHave++;
            if(timesHave==1){ 
                ...
                DocManager.Inst.ExecuteCmd(new TrackChangeSingerCommand(DocManager.Inst.Project, DocManager.Inst.Project.tracks[0], SingerManager.Inst.Singers[Program.singerName]));
                DocManager.Inst.EndUndoGroup();

                DocManager.Inst.StartUndoGroup();
                //var factory = DocManager.Inst.PhonemizerFactories.FirstOrDefault(factory => factory.type.FullName == "DiffSinger Rhythmizer Phonemizer");
                var factory = DocManager.Inst.PhonemizerFactories[12];
                var phonemizer = factory?.Create();
                if (phonemizer != null) {
                    DocManager.Inst.ExecuteCmd(new TrackChangePhonemizerCommand(DocManager.Inst.Project, DocManager.Inst.Project.tracks[0], phonemizer));
                } 
                DocManager.Inst.EndUndoGroup();

            //DocManager.Inst.Project.tracks[0].singer_ = SingerManager.Inst.Singers["JiHeHong-diffsinger-openutau-V2"];
                newWindow();
                pianoRollWindow?.Show();
                pianoRollWindow?.EditLyrics(Program.lyricsContent);
        ...
        async Task exportOutIt(){
            var project = DocManager.Inst.Project;
            var file = "C:\\Users\\yuanke\\Desktop\\OpenUtau-0.1.501.42\\bufferBlock\\output\\"+Program.toFileName;
        ...
        public void newWindow(){
            ...
            //if (control is PartControl partControl && partControl.part is UVoicePart) {
                ...
                DocManager.Inst.ExecuteCmd(new LoadPartNotification(DocManager.Inst.Project.parts[0], DocManager.Inst.Project, 0));
        ...
        async void OpenIt() {
            ...
            "C:\\Users\\yuanke\\Desktop\\OpenUtau-0.1.501.42\\bufferBlock\\input\\"+Program.midiName
            };
