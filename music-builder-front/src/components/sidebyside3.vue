<template>
  <div class="side-by-side-component">
    ...
    <!-- 添加el-dialog组件 -->
    <el-dialog v-model="dialogVisible" title="AI歌曲封面绘制" width="500">
...
dialogVisible.value = false; // 关闭弹窗
if (!form.value.keyword||!form.value.name) {
  alert('请填写完整数据')
  return
}
...
console.log(form.value)
try {
  const response = await http.post('render/three', 
      formData,{
    headers:{
